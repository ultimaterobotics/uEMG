...
//	NVIC_DisableIRQ(GPIOTE_IRQn);
	bmi_read_buf(BMI_DATA_GYRO, 15, bmi.raw_data);
//	NVIC_EnableIRQ(GPIOTE_IRQn);
	int16_t v;
...
void bmi160_normal_mode()
{
	...
	bmi_write_reg8(BMI_ACC_CONF, 0b10101000); //010 bwp, 100 Hz, undersampling enabled
	delay_ms(1);
	bmi_write_reg8(BMI_ACC_RANGE, 0b00000101); //+-4g
	bmi.rawA2SI = 9.8 * 4.0 / 32768.0;// 4g mode
	delay_ms(1);
	bmi_write_reg8(BMI_GYR_CONF, 0b00101000); //10 bwp, 100 Hz
	delay_ms(1);
//	bmi_write_reg8(BMI_GYR_RANGE, 0b00000010); //+-500 dps
	bmi_write_reg8(BMI_GYR_RANGE, 0b00000000); //+-2000 dps
//	bmi.rawG2SI = 0.000266048;// 500 dps mode
	bmi.rawG2SI = 0.001065264;// 2000 dps mode
	delay_ms(1);
